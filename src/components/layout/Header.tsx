...
const bebas = Bebas_Neue({
  weight: '400', // This is the weight that will be used for font-bebas. font-bold will attempt to use a bolder version if available or apply faux bold.
  subsets: ['latin'],
  variable: '--font-bebas', // Ensure this CSS variable is used in tailwind.config or globals.css
  display: 'swap',
});

const montserrat_nav_font = Montserrat({ // Renamed to avoid conflict if 'montserrat' is used elsewhere
  weight: ['500'], // Corresponds to font-medium
  subsets: ['latin'],
  variable: '--font-montserrat-nav', // Ensure this CSS variable is used for `font-montserrat_nav`
  display: 'swap',
});
...
export default function Header() {
  ...
  const [isDesktopSearchVisible, setIsDesktopSearchVisible] = useState(false); // For desktop search
  const [isScrolled, setIsScrolled] = useState(false);
  const wishlistCount = 2; // Example static wishlist count
  ...
  const toggleMobileMenu = () => {
    ...
    if (!isMobileMenuOpen) { // If opening mobile menu
      setIsProfileOpen(false); // Close profile dropdown
      // setIsDesktopSearchVisible(false); // Close desktop search if open
    }
  };
  ...
  const headerBgClass = isScrolled ? 'bg-white shadow-lg' : 'bg-black'; // Added shadow-lg for scrolled
  ...
  const iconHoverClasses = isScrolled
    ...
    : 'hover:bg-white/15 hover:text-gray-200'; // Slightly adjusted hover for dark bg
  ...
  const getNavLinkClasses = (scrolled: boolean) => {
    const fontClass = "font-montserrat_nav"; // Use the Tailwind font family name
    if (scrolled) {
  ...
  const getSpecialOffersLinkClasses = (scrolled: boolean) => {
    ...
    if (scrolled) {
      // Scrolled state: White background, special offer text color
      colorStyles = 'bg-white text-special-offer shadow-md hover:shadow-lg hover:scale-105 border border-special-offer/30';
    } else {
      // Not scrolled (on black header): Special offer background, white text
  ...
  return (
    <>
      {/* Apply font variables to the body or a high-level wrapper if you want them globally, 
          or apply .className to specific sections like <nav> */}
      <header className={`sticky top-0 z-50 transition-colors duration-300 ease-in-out ${headerBgClass} ${bebas.variable} ${montserrat_nav_font.variable}`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
          ...
          <Link
            href="/"
            className={`text-6xl lg:text-5xl font-bebas font-bold transition-colors duration-300 ease-in-out italic tracking-wider ${logoTextColorClass} ${logoHoverTextColorClass}`} // Added font-bold
          >
            PRIMATE
          </Link>

          {/* Desktop Navigation */}
          <nav className={`hidden md:flex items-center md:gap-1 lg:gap-2 font-montserrat_nav`}> {/* Applied font here */}
            {/* "Top" Link */}
            ...
            <Link
              href="/special-offers"
              className={`${getSpecialOffersLinkClasses(isScrolled)}`} // Removed animate-tag-shake
            >
              Special Offers
              <span className="absolute -top-1.5 -right-1.5 flex h-3.5 w-3.5"> {/* Adjusted size/position */}
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-special-offer opacity-75 group-hover:opacity-100"></span> {/* Changed to animate-ping */}
              ...
              {isDesktopSearchVisible && (
                <div className="absolute right-0 top-full mt-2 z-20">
                  <input
                    ...
                    autoFocus
                    onBlur={() => setTimeout(() => setIsDesktopSearchVisible(false), 100)} // Close on blur with delay
                  />
                </div>
              )}
            </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-40 border-t pb-4">
            <nav className={`flex flex-col gap-1 px-4 pt-4 text-gray-800 font-montserrat_nav`}> {/* Applied font here */}
              <div className="mb-3">
                <input
                  type="search"
                  placeholder="Search products..."
              ...
              {allCategoriesForMobile.map((category) => (
                <Link
                  key={category.id}
                  href={`/category/${category.id}`} // Ensure this path is correct
              ...
              >
                Special Offers
                 <span className="absolute top-1/2 -translate-y-1/2 right-4 flex h-2.5 w-2.5 opacity-80 group-hover:opacity-100">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-special-offer opacity-75"></span> {/* Changed to animate-ping */}
